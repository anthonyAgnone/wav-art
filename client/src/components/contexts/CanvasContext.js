    ...
    display = ({ audioData, stdv, average }) => {
        ...
        function Line(x1, y1, x2, y2, dx1, dy1, dx2, dy2, color, ctx) {
            ...
            this.update = function ({ audioData }) {
                ...
                // console.log(audioData[19])
                // console.log(audioData[29])
                // console.log(audioData[9])
        ...
        if (audioData[40] > 200 && this.lineArray.length < 3) {
            let x1 = canvas.width * .5;
            let y1 = canvas.height * .5;
            let x2 = canvas.width * .5;
            let y2 = canvas.height * .5;

            let dx1 = (stdv * Math.PI - Math.floor(stdv * Math.PI) - .5) * stdv * 10

            let dy1 = (stdv - Math.floor(stdv) - .5) * 3

            let dx2 = (stdv * 2.0923789 - Math.floor(stdv * 2.0923789) - .5) * 2

            let dy2 = (stdv * 5.9823 - Math.floor(stdv * 5.9823) - .5) * stdv * 10

            let color = `rgba(${audioData[19]},${audioData[29]},${audioData[9]}, )`;


            this.lineArray.push(
                new Line(x1, y1, x2, y2, dx1, dy1, dx2, dy2, color, this.ctx)
            )
        }
        ...
        if (stdv > 13) {
            ...
            this.ctx.fillRect(canvas.width * (stdv * Math.PI - (Math.floor(stdv * Math.PI))), canvas.height * fakeRandom, 50, 50)
            ...
            this.ctx.fillRect(-2 * audioData[300] + 300, 2 * audioData[350] + 300, audioData[0] * .2, audioData[2] * 2)
